...
class customersProcess
{
  ...
  private function create()
  {
    try {
      ...
      $testimony = htmlspecialchars($_REQUEST['testimony'], ENT_QUOTES);
      ...
      $tipo_visa = htmlspecialchars($_REQUEST['tipo_visa'], ENT_QUOTES);
      $interes = htmlspecialchars($_REQUEST['interes'], ENT_QUOTES);

      // Obtenemos prefijo de país
      $countryCode = $locationDAO->getCountryById($country)->fetch_object();
      $phoneUtil = PhoneNumberUtil::getInstance();
      $code = $phoneUtil->getCountryCodeForRegion($countryCode->code);

      // Create register
      $response = $customersDAO->create($name, $code.$phone, $email, $testimony, $year, $city, $passport, $tipo_visa, $interes);
